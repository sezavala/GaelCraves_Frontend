...
export const NON_PROXY_REDIRECT_URI = makeRedirectUri({ useProxy: false } as any);
    ...
    if (response?.type === 'success') {
      ...
      // If we got an id_token (some flows/platforms), decode it to get email
        ...
        try {
          ...
          if (emailFromId) {
            try { setUser({ email: emailFromId }); } catch(e) { console.warn('setUser failed', e); }
            if (Platform.OS === 'web') {
              window.localStorage.setItem('@user', JSON.stringify({ email: emailFromId }));
  ...
  if (code) {
        ...
        (async () => {
          try {
            ...
            const email = profile.email || profile.email_address || profile.preferred_email || profile.emailAddress;
            if (email) {
              setUser({ email });
              if (Platform.OS === 'web') {
        ...
        console.warn('No authorization code returned by Google', response.params);
      }
    }
        ...
        if (codeFromUrl) {
          ...
          (async () => {
            try {
              ...
              const profile = await resp.json();
              const email = profile.email || profile.email_address || profile.preferred_email;
              if (email) {
                setUser({ email });
              } else {
                console.warn('No email in profile returned by backend', profile);
